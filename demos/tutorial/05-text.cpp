...
createSampleDoc()
{
  ...
  svg::SVGTextElement * text = new svg::SVGTextElement(doc);

  text->setFill(0,.6,0);
  text->setText("svgl is so cool");
  text->setFontFamily(unicode::String::internString("helvetica"));
  text->setFontSize(30);
  text->setFontStyle(glft::FontInfo::fs_italic);
  text->setFontWeight(glft::FontInfo::fw_bold);
  // order independant
